...
class TelegramWatchdog:
    ...
    def _check_bot_running(self):
        ...
        try:
            ...
            # 2. Verificar proceso con psutil
            import psutil
            ...
            # En caso de error, asumir que no corre para evitar bloqueos, 
            # PERO el error 409 manejará el conflicto real.
            ...
            try:
                ...
                if bot_running:
                    ...
                    if self.polling_active:
                        print("👀 ✅ Bot detectado activo (PID existe). Pausando watchdog polling completamente.")
                        self.polling_active = False
...
try:
    ...
    # Fallback si hay error con el radio
...
if page == "🏠 Inicio":
    ...
    # Bot Status with enhanced styling
    status_color = "#4caf50" if bot_running else "#f44336"
    status_text = "ACTIVO" if bot_running else "INACTIVO"
    status_icon = "🟢" if bot_running else "🔴"
    col3.markdown(f"""
    <div class="metric-card fade-in">
        <div style="font-size: 0.9rem; color: #666; margin-bottom: 0.5rem;">🤖 Estado del Bot</div>
        <div style="font-size: 1.5rem; margin-bottom: 0.25rem;">
            <span style="font-size: 2rem;">{status_icon}</span>
            <span style="font-weight: 800; color: {status_color}; margin-left: 0.5rem;">{status_text}</span>
        </div>
        <div style="font-size: 0.8rem; color: {status_color};">
            {'✅ Operativo' if bot_running else '⏸️ Detenido'}
        </div>
    </div>
    """, unsafe_allow_html=True)
...
# ==================== PAGE: TERMINAL DE TRADING ====================
    ...
    # --- TAB 2: BOT AUTOMÁTICO ---
    with tab_auto:
        st.subheader("🤖 Control del Bot Autónomo")
        st.info("El bot analiza automáticamente el mercado, detecta señales y ejecuta operaciones según las reglas de riesgo configuradas. Todas las operaciones se registran con historial de IOL para cálculo preciso de ganancias/pérdidas.")
        
        PID_FILE = "bot.pid"
        # Usar la misma función compartida para verificar estado
        ...
        with col_control1:
            if running:
                st.success(f"🟢 Bot ACTIVO (PID: {pid})")
                ...
                st.warning("🔴 Bot INACTIVO")
                mode = st.radio("Modo de Operación", ["🧪 Paper Trading", "💰 Live Trading"], horizontal=True)
                paper_mode = "🧪" in mode
                
                interval = st.number_input("Intervalo de Análisis (minutos)", min_value=1, max_value=1440, value=60, help="Cada cuántos minutos el bot analiza el mercado")
                
                if st.button("🚀 Iniciar Bot", type="primary"):
                    if paper_mode:
                        # Usar run_bot.py wrapper para mejor manejo de errores
                        cmd = [sys.executable, 'run_bot.py', '--continuous', '--interval', str(interval)]
                    else:
                        cmd = [sys.executable, 'run_bot.py', '--continuous', '--interval', str(interval), '--live']
                    
                    try:
                        if sys.platform == 'win32':
                            # CREATE_NEW_CONSOLE crea una nueva ventana de consola
                            # El wrapper run_bot.py manejará los errores y mantendrá la ventana abierta
                        ...
                        st.success(f"✅ Bot iniciado en modo {'Paper Trading' if paper_mode else 'LIVE TRADING'}")
                        st.rerun()
                    except Exception as e:
            ...
            if os.path.exists('trades.json'):
                try:
                    with open('trades.json', 'r', encoding='utf-8') as f:
                        ...
                        st.metric("Ventas con P&L", len(sales_with_pnl))
        ...
        with col_control3:
            ...
            monitored = get_monitored_symbols()
            if monitored:
                st.write(", ".join([f"`{s}`" for s in monitored[:10]]))
                if len(monitored) > 10:
                    st.caption(f"... y {len(monitored) - 10} más")
            else:
                st.warning("No hay activos en la base de datos de entrenamiento.")
...
# ==================== PAGE: APRENDIZAJE CONTINUO ====================
    ...
    st.markdown("---")
    if st.button("🔄 Ejecutar Ciclo de Aprendizaje", type="primary"):
        ...
        **Configuración de monitoreo:**
        ...
        if st.button("💾 Guardar Configuración", type="primary"):
            ...
            try:
                ...
                st.info("💡 Los cambios se aplicarán la próxima vez que inicies el bot")
...
# ==================== PAGE: SISTEMA & CONFIGURACIÓN ====================
    ...
    tab_train, tab_growth, tab_risk, tab_sentiment, tab_telegram, tab_reports, tab_logs = st.tabs(["🧠 Entrenamiento IA", "📈 Monitoreo de Crecimiento", "🛡️ Gestión de Riesgo", "💭 Análisis de Sentimiento", "📱 Telegram", "📊 Reportes Diarios", "📝 Logs"])
