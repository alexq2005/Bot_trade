...
class TechnicalAnalysisService:
    ...
    def calculate_volatility_indicators(self, df, config=None):
        """
        Calculate volatility indicators with dynamic config.
        Returns dict with ATR, Bollinger Bands, etc.
        """
        cfg = config or {}
        atr_window = cfg.get('atr_period', 14)
        bb_window = cfg.get('bb_window', 20)
        bb_dev = cfg.get('bb_dev', 2)
        ...
        # Bollinger Bands
        bb_indicator = ta.volatility.BollingerBands(df["close"], window=bb_window, window_dev=bb_dev)
    ...
    def calculate_momentum_indicators(self, df, config=None):
        """
        Calculate momentum indicators (RSI, MACD, Stochastic).
        """
        cfg = config or {}
        rsi_window = cfg.get('rsi_period', 14)
        
        # RSI
        rsi_indicator = ta.momentum.RSIIndicator(df["close"], window=rsi_window)
    ...
    def calculate_trend_indicators(self, df, config=None):
        """
        Calculate trend indicators (SMA, EMA, ADX).
        """
        cfg = config or {}
        sma_fast_window = cfg.get('sma_fast', 20)
        sma_slow_window = cfg.get('sma_slow', 50)
        
        # Moving Averages
        sma_20 = ta.trend.SMAIndicator(df["close"], window=sma_fast_window).sma_indicator()
        sma_50 = ta.trend.SMAIndicator(df["close"], window=sma_slow_window).sma_indicator()
    ...
    def get_full_analysis(self, symbol, config=None):
        """
        Get complete technical analysis for a symbol.
        Integrates real-time data from IOL if available.
        Allows dynamic configuration for genetic optimization.
        """
        ...
        volatility = self.calculate_volatility_indicators(df, config)
        momentum = self.calculate_momentum_indicators(df, config)
        trend = self.calculate_trend_indicators(df, config)

        # Generate trading signal based on indicators
        signal = self._generate_signal(volatility, momentum, trend, config)
    ...
    def _generate_signal(self, volatility, momentum, trend, config=None):
        """
        Generate trading signal based on technical indicators.
        Uses dynamic thresholds from config if available.
        """
        signals = []
        cfg = config or {}
        ...
        if momentum["rsi"] is not None:
            if momentum["rsi"] < rsi_oversold:
                signals.append("BUY")  # Oversold
            elif momentum["rsi"] > rsi_overbought:
