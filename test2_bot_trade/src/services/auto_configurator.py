...
class AutoConfigurator:
    ...
    def _analyze_signals_from_operations(self) -> Dict:
        """MEJORA #5: Analiza señales generadas desde operations_log.json (no solo trades ejecutados)"""
        operations_log_file = Path("data/operations_log.json")
        ...
        try:
            ...
            logger.error(f"Error leyendo operations_log.json para auto-configuración: {e}")
            return {}
        ...
        # Analizar últimas 100 señales
        recent_analyses = analyses[-100:] if len(analyses) > 100 else analyses
        
        signal_stats = {
            'total_signals': len(recent_analyses),
            'buy_signals': 0,
            'sell_signals': 0,
            'hold_signals': 0,
            'scores_near_threshold': 0,  # Scores entre 25-35 (cerca del umbral)
            'scores_just_below': 0,  # Scores entre 20-25 (justo debajo)
            'scores_just_above': 0,  # Scores entre 35-40 (justo arriba)
            'avg_score': 0,
            'max_score': 0,
            'min_score': 0
        }
        ...
        for analysis in recent_analyses:
            ...
            # Analizar proximidad a umbrales
            buy_threshold = self.config.get('buy_threshold', 35)
            if 20 <= score < buy_threshold:
                signal_stats['scores_just_below'] += 1
            elif buy_threshold <= score < buy_threshold + 5:
                signal_stats['scores_just_above'] += 1
            elif abs(score - buy_threshold) <= 10:
                signal_stats['scores_near_threshold'] += 1
        
        if scores:
            signal_stats['avg_score'] = sum(scores) / len(scores)
            signal_stats['max_score'] = max(scores)
            signal_stats['min_score'] = min(scores)
        
        return signal_stats
    
    def proactive_auto_configure(self) -> Dict:
        """MEJORA #5: Auto-configuración proactiva basada en señales generadas"""
        signal_stats = self._analyze_signals_from_operations()
        
        if not signal_stats or signal_stats['total_signals'] < 20:
            return {'changes': [], 'reason': 'Insuficientes señales para análisis'}
        
        changes = []
        old_config = self.config.copy()
        
        # 1. Ajustar umbral de compra si hay muchas señales cerca pero no alcanzan
        buy_threshold = self.config.get('buy_threshold', 35)
        just_below_ratio = signal_stats['scores_just_below'] / signal_stats['total_signals']
        buy_ratio = signal_stats['buy_signals'] / signal_stats['total_signals']
        
        if just_below_ratio > 0.15 and buy_ratio < 0.05:
            # Muchas señales justo debajo del umbral y pocas BUY
            # Reducir umbral ligeramente para capturar más oportunidades
            new_threshold = max(25, buy_threshold - 3)
            if new_threshold != buy_threshold:
                self.config['buy_threshold'] = new_threshold
                changes.append(f"Umbral de compra: {buy_threshold} → {new_threshold} (muchas señales cerca del umbral)")
        
        elif buy_ratio > 0.20:
            # Demasiadas señales BUY, ser más selectivo
            new_threshold = min(40, buy_threshold + 2)
            if new_threshold != buy_threshold:
                self.config['buy_threshold'] = new_threshold
                changes.append(f"Umbral de compra: {buy_threshold} → {new_threshold} (demasiadas señales BUY)")
        
        # 2. Ajustar umbral de venta simétricamente
        sell_threshold = self.config.get('sell_threshold', -35)
        if 'buy_threshold' in [c for c in changes if 'Umbral de compra' in c]:
            new_sell_threshold = -self.config['buy_threshold']
            if new_sell_threshold != sell_threshold:
                self.config['sell_threshold'] = new_sell_threshold
                changes.append(f"Umbral de venta: {sell_threshold} → {new_sell_threshold} (ajuste simétrico)")
        
        # 3. Ajustar máximo de trades diarios según actividad
        max_daily_trades = self.config.get('max_daily_trades', 10)
        avg_score = signal_stats['avg_score']
        
        if avg_score > 20 and buy_ratio > 0.10:
            # Buena actividad, permitir más trades
            new_max = min(15, max_daily_trades + 2)
            if new_max != max_daily_trades:
                self.config['max_daily_trades'] = new_max
                changes.append(f"Máximo trades/día: {max_daily_trades} → {new_max} (buena actividad detectada)")
        elif avg_score < 5 and buy_ratio < 0.03:
            # Poca actividad, reducir trades
            new_max = max(5, max_daily_trades - 2)
            if new_max != max_daily_trades:
                self.config['max_daily_trades'] = new_max
                changes.append(f"Máximo trades/día: {max_daily_trades} → {new_max} (poca actividad)")
        
        if changes:
            self.save_config()
            # Usar el método existente _log_configuration_change
            performance_data = {
                'signal_stats': signal_stats,
                'total_signals': signal_stats.get('total_signals', 0),
                'buy_ratio': signal_stats.get('buy_signals', 0) / signal_stats.get('total_signals', 1) if signal_stats.get('total_signals', 0) > 0 else 0
            }
            self._log_configuration_change(old_config, self.config, changes, performance_data)
            logger.info(f"✅ Auto-configuración proactiva: {len(changes)} cambios aplicados")
        
        return {
            'changes': changes,
            'signal_stats': signal_stats,
            'reason': 'Análisis proactivo de señales generadas'
        }
